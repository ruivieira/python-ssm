...
from nose.tools import assert_true, assert_equal  # type: ignore
import numpy as np
...
class DglmTests(unittest.TestCase):
    ...
    def univariate_lc_prior_dimensions_test(self):
        ...
        lc = UnivariateStructure.locally_constant(1.0)
        dlm = NormalDLM(structure=lc, V=1.4)
        assert_equal(dlm.current_state.shape, (1,), "Prior has the wrong dimensions")
    ...
    def univariate_ll_prior_dimensions_test(self):
        ...
        assert_equal(dlm.current_state.shape, (2,), "Prior has the wrong dimensions")
    ...
    def univariate_ll_iterator_dimensions_test(self):
        ...
        print(items)
        assert_equal(items[0][0].shape, (2,), "State has the wrong dimensions")
        assert_true(np.isscalar(items[0][1]), "Observation has the wrong dimensions")
